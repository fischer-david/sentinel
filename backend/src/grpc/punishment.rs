...
impl GrpcPunishmentService {
    pub fn new(punishment_service: Arc<PunishmentService>, message_service: Arc<MessageService>,
               broadcast_service: Arc<BroadcastService>) -> Self {
        Self {
            punishment_service,
            message_service,
            broadcast_service,
...
impl GrpcPunishmentService {
    async fn handle_player_status_change(
        broadcast_handler: &BroadcastHandler<Uuid, PunishmentWithTemplate>, // Adjust type as needed
        identifier: &Uuid,
        ...
        if request.online {
            Ok(broadcast_handler.add_key_to_listener(identifier, player_id)
                .await)
        } else {
            Ok(broadcast_handler.remove_key_from_listener(identifier, player_id)
                .await)
        }
    }
    ...
    async fn create_punishment_response(
        ...
        let reason = punishment.custom_reason.as_ref()
            .unwrap_or(&punishment.reason_template);

        let disconnect_message = if punishment_type.contains("ban") || punishment_type.contains("kick") {
            Some(DisconnectMessage {
                message: message_service.get_ban_message(reason, punishment.issued_at, punishment.expires_at)
                    .await.map_err(|e| format!("Failed to get ban message: {}", e))?
            })
        } else {
            None
        };
        ...
        let chat_message = if punishment_type.contains("mute") || punishment_type.contains("warn") {
            Some(ChatMessage {
                message: message_service.get_mute_message(reason, punishment.expires_at)
                    .await.map_err(|e| format!("Failed to get mute message: {}", e))?
            })
        } else {
            None
        };
...
impl GeneratedPunishmentService for GrpcPunishmentService {
    async fn get_player_login(&self, request: Request<GetPlayerLoginRequest>) -> Result<Response<GetPlayerLoginResponse>, Status> {
        let request = request.into_inner();
        let punishments = self.punishment_service.get_active_punishments(&request.player_id)
            .await.map_err(|e| Status::internal(format!("Failed to get active punishments: {}", e)))?;

        let grpc_punishments: Vec<Punishment> = punishments.iter().map(|punishment| punishment.clone().into()).collect();
        ...
        for punishment in punishments {
            ...
            if disconnect_message.is_none() && punishment_type.contains("ban") {
                let reason = if let Some(reason) = &punishment.custom_reason {
                    reason
                } else {
                    &punishment.reason_template
                };

                disconnect_message = Some(DisconnectMessage {
                    message: self.message_service.get_ban_message(&reason, punishment.issued_at, punishment.expires_at)
                        .await.map_err(|e| Status::internal(format!("Failed to get ban message: {}", e)))?
                });
            }

            if chat_message.is_none() && (punishment_type.contains("mute") || punishment_type.contains("warn")) {
                let reason = if let Some(reason) = &punishment.custom_reason {
                    reason
                } else {
                    &punishment.reason_template
                ...
                chat_message = Some(ChatMessage {
                    message: self.message_service.get_mute_message(&reason, punishment.expires_at)
                        .await.map_err(|e| Status::internal(format!("Failed to get mute message: {}", e)))?
                });
            }
    ...
    async fn get_live_punishments(&self, request: Request<Streaming<GetLivePunishmentsRequest>>) -> Result<Response<Self::GetLivePunishmentsStream>, Status> {
        ...
        tokio::spawn(async move {
            while let Some(result) = request_stream.next().await {
                match result {
                    Ok(request) => {
                        if let Err(e) = Self::handle_player_status_change(&broadcast_handler_for_requests, &identifier, &request).await {
                            eprintln!("Error handling player status change: {}", e);
                            let _ = tx_for_cleanup.send(Err(Status::internal("Failed to update player status"))).await;
                            break;
                        }
                    },
                    Err(e) => {
                        eprintln!("Error in request stream: {}", e);
                        let _ = tx_for_cleanup.send(Err(Status::internal("Request stream error"))).await;
                        break;
                    }
                }
            }
        ...
        tokio::spawn(async move {
            while let Ok(punishment_broadcast) = broadcast_rx.recv().await {
                match Self::create_punishment_response(&message_service, &punishment_broadcast.value).await {
                    Ok(response) => {
                    ...
                    Err(e) => {
                        ...
                        let _ = tx_for_broadcast.send(Err(Status::internal("Failed to process punishment"))).await;
                        break;
                    }
                }
            }
